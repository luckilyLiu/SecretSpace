...
abstract class BaseAdapter<T> : Adapter<BaseHolder<T, ViewBinding, BaseAdapter<T>>>() {
    private val data = mutableListOf<T>()
    var itemClickListener: BaseItemClickListener<T>? = null
    var itemLongClickListener: BaseItemLongClickListener<T>? = null

    override fun onBindViewHolder(holder: BaseHolder<T, ViewBinding, BaseAdapter<T>>, position: Int) {
        holder.bindView(data[position], position)
        itemClickListener?.let { tmpItemClickListener ->
            holder.itemView.setOnClickListener {
                tmpItemClickListener.onClick(holder, data[position], position)
            }
        }
        itemLongClickListener?.let { tmpItemLongClickListener ->
            holder.itemView.setOnLongClickListener {
                return@setOnLongClickListener tmpItemLongClickListener.onLongClick(
                    holder.binding,
                    data[position],
                    position
                )
